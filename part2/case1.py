...
def max_over_all_actions(itr,pos,mat,arr,sta,hea):
    ...
    for act in range(action):
        ...
        # east square (IJ will be affected by MM's attack now) 
            ...
            # ready state (sta=1)  
                ...
                # movement
                if act == 2:
                    value = (-step_cost -0.5*bad_reward  + gamma*(1.0*(0.5*utility[itr-1,pos,mat,0,0,min(100,hea+25)] + 0.5*utility[itr-1,0,mat,arr,1,hea])))
                elif act == 4:
    ...
    f = open("part_2_task_2.1_trace.txt", "a")
